...
(function() {
  var API_ID, API_Key, Dasboard_URL, Evercam_API_URL, autoCreateCameraId, autoLogInDashboard, cleanAndSetJpegUrl, clearForm, createCamera, createUserAccount, getAPICredentials, gotSnapshot, handleContinueBtn, handleInputEvents, handleVendorModelEvents, handleWindowResize, hasCameraInfo, hasModelImage, iframeWindow, initAddCamera, initLadda, loadVendorModels, loadVendors, loader, onClickTabs, resizeIframe, sortByKey, switchTab, testSnapshot, useAuthentication, validAllInformation, validateEmail, validateInt, validate_hostname;
  ...
  loadVendors = function() {
    ...
    onSuccess = function(result, status, jqXHR) {
      var j, len, results, vendor, vendors;
      vendors = sortByKey(result.vendors, "name");
      results = [];
      for (j = 0, len = vendors.length; j < len; j++) {
        vendor = vendors[j];
        results.push($("#camera-vendor").append("<option value='" + vendor.id + "'>" + vendor.name + "</option>"));
      }
      return results;
    };
    settings = {
  ...
  loadVendorModels = function(vendor_id) {
    ...
    onSuccess = function(result, status, jqXHR) {
      var default_password, default_username, j, jpg_url, len, model, models, selected_option;
      $("#camera-model option").remove();
      ...
      for (j = 0, len = models.length; j < len; j++) {
        model = models[j];
        jpg_url = model.defaults.snapshots && model.defaults.snapshots.jpg.toLowerCase() !== "unknown" ? model.defaults.snapshots.jpg : '';
        default_username = model.defaults.auth !== null && model.defaults.auth !== void 0 ? model.defaults.auth.basic.username : '';
        default_password = model.defaults.auth !== null && model.defaults.auth !== void 0 ? model.defaults.auth.basic.password : '';
        if (model.name.toLowerCase().indexOf('default') !== -1) {
      ...
      if ($("#camera-model").find(":selected").attr("jpg-val") !== 'Unknown') {
        ...
        return $("#camera-snapshot-url").removeClass("invalid").addClass("valid");
      }
    };
    settings = {
      cache: false,
  ...
  hasModelImage = function(vendor_id, model_id) {
    var image_url, img;
    img = new Image();
    image_url = "http://evercam-public-assets.s3.amazonaws.com/" + vendor_id + "/" + model_id + "/thumbnail.jpg";
    img.onload = function() {
      return $("#model-image").attr("src", image_url);
    };
    img.onerror = function() {
      return $("#model-image").attr("src", "/assets/plain.png");
    };
  ...
  handleVendorModelEvents = function() {
    $("#camera-vendor").on("change", function() {
      ...
      img.onload = function() {
        return $("#vemdor-image").attr("src", image_url);
      };
      img.onerror = function() {
        return $("#vemdor-image").attr("src", "/assets/plain.png");
      };
      img.src = image_url;
      return loadVendorModels($(this).val());
    });
    return $("#camera-model").on("change", function() {
      var selected_option, snapshot_url;
      selected_option = $(this).find(":selected");
      hasModelImage($("#camera-vendor").val(), $(this).val());
      snapshot_url = selected_option.attr("jpg-val");
      $("#default-username").text(selected_option.attr("username-val"));
      $("#default-password").text(selected_option.attr("password-val"));
      if (snapshot_url !== 'Unknown') {
        return cleanAndSetJpegUrl(snapshot_url);
      }
    });
  };

  cleanAndSetJpegUrl = function(jpeg_url) {
    if (jpeg_url.indexOf('/') === 0) {
      jpeg_url = jpeg_url.substr(1);
    }
    return $("#camera-snapshot-url").val(jpeg_url);
  };

  useAuthentication = function() {
    return $("#required-authentication").on('click', function() {
      if ($(this).is(":checked")) {
        return $("#authentication").removeClass("hide");
      } else {
        return $("#authentication").addClass("hide");
      }
    });
  };

  handleInputEvents = function() {
    $("#camera-url").on('keyup', function(e) {
      if (validate_hostname($(this).val())) {
        $(this).removeClass("invalid").addClass("valid");
      } else {
        $(this).removeClass("valid").addClass("invalid");
      }
      return validAllInformation();
    });
    $("#camera-url").on('focus', function(e) {
      $(".info-box .info-header").text("EXTERNAL IP / URL");
      return $(".info-box .info-text").text("Put the public URL or IP address of your camera. You will need to have setup port forwarding for your camera.");
    });
    $(".external-url").on('click', function() {
      $(".info-box .info-header").text("EXTERNAL IP / URL");
      return $(".info-box .info-text").text("Put the public URL or IP address of your camera.");
    });
    $("#camera-port").on('keyup', function(e) {
    ...
    $("#camera-port").on('focus', function(e) {
      $(".info-box .info-header").text("EXTERNAL PORT");
      return $(".info-box .info-text").text("The port should be a 2-5 digit number. The default external port is 80.");
    });
    $(".port").on('click', function() {
      $(".info-box .info-header").text("EXTERNAL PORT");
      return $(".info-box .info-text").text("The port should be a 2-5 digit number. The default external port is 80.");
    });
    $("#camera-snapshot-url").on('keyup', function(e) {
      if ($(this).val() === '') {
        $(this).removeClass("valid").addClass("invalid");
      } else {
        $(this).removeClass("invalid").addClass("valid");
    ...
    $("#camera-snapshot-url").on('focus', function(e) {
      $(".info-box .info-header").text("SNAPSHOT URL");
      return $(".info-box .info-text").text("If you know your Camera Vendor & Model we can work this out for you. You can also enter it manually for your camera.");
    });
    $(".snapshot-url").on('click', function() {
      $(".info-box .info-header").text("SNAPSHOT URL");
      return $(".info-box .info-text").text("If you know your Camera Vendor & Model we can work this out for you. You can also enter it manually for your camera.");
    });
    $("#camera-name").on('keyup', function(e) {
      return $(this).removeClass("invalid").addClass("valid");
    });
    $("#camera-id").on('keyup', function(e) {
      return $(this).removeClass("invalid").addClass("valid");
    });
    $("#user-email").on('keyup', function(e) {
      if (validateEmail($(this).val())) {
        ...
        return $(this).removeClass("valid").addClass("invalid");
      }
    });
    ...
    $(".default-username").on('click', function() {
      return $("#camera-username").val($("#default-username").text());
    });
    return $(".default-password").on('click', function() {
      return $("#camera-password").val($("#default-password").text());
    });
  };
  ...
  validateEmail = function(email) {
    ...
    return reg.test(emailtrimed);
  };

  validAllInformation = function() {
    if ($("#camera-port") === '') {
      if ($("#camera-url").hasClass('valid') && $("#camera-snapshot-url").hasClass('valid')) {
        $(".test-image").removeClass('hide');
        return $(".help-texts").addClass('hide');
      } else {
        $(".test-image").addClass('hide');
        return $(".help-texts").removeClass('hide');
      }
    } else {
  ...
  testSnapshot = function() {
    return $("#test-snapshot").on('click', function() {
      var data, onError, onSuccess, port, settings;
      initLadda(this);
      if ($("#camera-port").val() !== '') {
        port = $("#camera-port").val();
      }
      data = {};
      data.external_url = "http://" + ($('#camera-url').val()) + ":" + port;
      data.jpg_url = $('#camera-snapshot-url').val();
      if ($("#camera-username").val() !== '') {
        data.cam_username = $("#camera-username").val();
      }
      if ($("#camera-password").val() !== '') {
        data.cam_password = $("#camera-password").val();
      }
      onError = function(jqXHR, status, error) {
  ...
  handleContinueBtn = function() {
    $("#continue-step2").on('click', function() {
      return switchTab("camera-details", "camera-info");
    });
    return $("#continue-step3").on('click', function() {
      if ($("#camera-name").val() === '') {
      ...
      return switchTab("camera-info", "user-create");
    });
  };

  autoCreateCameraId = function() {
    return $("#camera-name").on('keyup', function() {
      return $("#camera-id").val($(this).val().replace(RegExp(" ", "g"), "-").toLowerCase());
    });
  };

  hasCameraInfo = function() {
    if ($("#camera-url").val() === '' && $("#camera-snapshot-url").val() === '') {
    ...
    if ($("#camera-name").val() === '' && $("#camera-id").val() === '') {
      $("#camera-name").removeClass("valid").addClass("invalid");
      $("#camera-id").removeClass("valid").addClass("invalid");
  ...
  createUserAccount = function() {
    return $("#create-account").on('click', function() {
      var data, onError, onSuccess, settings;
      if ($("#username").val() === '') {
  ...
  getAPICredentials = function() {
    ...
    onSuccess = function(result, status, jqXHR) {
      ...
      return createCamera(result.api_id, result.api_key);
    };
    settings = {
      cache: false,
      data: data,
  ...
  createCamera = function(api_id, api_key) {
    var data, onDuplicateError, onError, onSuccess, settings;
    data = {};
    data.name = $("#camera-name").val();
    if ($("#camera-vendor").val() !== '') {
    ...
    if ($("#camera-port").val() !== '') {
      data.external_http_port = $("#camera-port").val();
    }
    data.jpg_url = $("#camera-snapshot-url").val();
    onError = function(jqXHR, status, error) {
    ...
    onSuccess = function(result, status, jqXHR) {
      return parent.location.href = Dasboard_URL + "/v1/cameras?api_id=" + api_id + "&api_key=" + api_key;
    };
    onDuplicateError = function(xhr) {
      switchTab("user-create", "camera-info");
    ...
    settings = {
      ...
      success: onSuccess,
      statusCode: {
        409: onDuplicateError,
        400: onDuplicateError
      },
      contentType: "application/x-www-form-urlencoded",
      type: 'POST',
  ...
  onClickTabs = function() {
    return $(".nav-steps li").on('click', function() {
      ...
      return $("" + previousTab).fadeOut(300, function() {
        return $("" + currentTab).fadeIn(300);
      });
    });
  };

  switchTab = function(hideTab, showTab) {
    $(".nav-steps li").removeClass('active');
    $("#" + hideTab).fadeOut(300, function() {
      return $("#" + showTab).fadeIn(300);
    });
    return $("#li-" + showTab).addClass('active');
  };

  initAddCamera = function() {
    var embedCode, iframe, url;
  ...
  resizeIframe = function(iframeControl) {
  ...
  window.initializeAddCameraPublic = function() {
    useAuthentication();
    loadVendors();
  ...
  $(window, document, void 0).ready(function() {
    var wskCheckbox;
    wskCheckbox = (function() {
      var SPACE_KEY, addEventHandler, clickHandler, clickHandlerLabel, findCheckBoxes, keyHandler, wskCheckboxes;
      wskCheckboxes = [];
      SPACE_KEY = 32;
      ...
      keyHandler = function(e) {
        ...
        if (e.keyCode === SPACE_KEY) {
          ...
          cbox = document.getElementById(this.parentNode.getAttribute('for'));
          cbox.checked = !cbox.checked;
        }
      };
      clickHandlerLabel = function(e) {
        var i, id;
      ...
      findCheckBoxes = function() {
        var i, labels, posCheckbox, span, text;
