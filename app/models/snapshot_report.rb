class SnapshotReport < ActiveRecord::Base
  ...
  def self.add_report
    camera_report = Snapshot.connection.select_all("select camera_id,count(*) as snapshot_count from snapshots where snapshot_id in (select snapshot_id from snapshots where created_at = 'now'::date - 1) group by camera_id").to_hash
