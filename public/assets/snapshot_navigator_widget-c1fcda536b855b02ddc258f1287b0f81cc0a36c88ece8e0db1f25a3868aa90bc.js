...
(function() {
  ...
  datePickerSelect = function(value) {
    ...
    hasDayRecord = false;
    for (k = 0, len = BoldDays.length; k < len; k++) {
      j = BoldDays[k];
      if (j === dt.getDate()) {
        hasDayRecord = true;
        break;
  ...
  SetInfoMessage = function(currFrame, date_time) {
    ...
    if ($(".nav-tabs li.active a").html() === "Recordings" && history.replaceState) {
  ...
  fullscreenImage = function() {
    $("#imgPlayback").dblclick(function() {
      return screenfull.toggle($(this)[0]);
    });
    if (screenfull.enabled) {
      return document.addEventListener(screenfull.raw.fullscreenchange, function() {
        if (screenfull.isFullscreen) {
          return $("#imgPlayback").css('width', 'auto');
        } else {
          return $("#imgPlayback").css('width', '100%');
        }
      });
    }
  };
  ...
  HighlightCurrentMonthSuccess = function(results, status, jqXHR) {
    ...
    return calDays.each(function() {
      var calDay, iDay, k, len, ref, result, results1;
      calDay = $(this);
      if (!calDay.hasClass('old') && !calDay.hasClass('new')) {
        iDay = parseInt(calDay.text());
        ref = results.days;
  ...
  BoldSnapshotHourSuccess = function(result, context) {
    var hasRecords, hour, k, lastBoldHour, len, ref;
    lastBoldHour = 0;
    hasRecords = false;
    ref = result.hours;
    for (k = 0, len = ref.length; k < len; k++) {
  ...
  SetPlayFromImage = function(timestamp) {
    var i, k, len, snapshot, snapshot_timestamp;
    i = 0;
    for (k = 0, len = snapshotInfos.length; k < len; k++) {
      snapshot = snapshotInfos[k];
  ...
  SelectImagesByMinSec = function() {
    var estimatedIndex, i, min, results1, sDt, sec, si;
  ...
  handleTabOpen = function() {
    return $('.nav-tab-recordings').on('show.bs.tab', function() {
      var date_time, url;
  ...
  saveImage = function() {
    return $('#save-recording-image').on('click', function() {
      var date_time;
      date_time = new Date(snapshotInfos[snapshotInfoIdx].created_at * 1000);
      return SaveImage.save($("#imgPlayback").attr('src'), Evercam.Camera.id + "-" + (getSnapshotDate(date_time).toISOString()) + ".jpg");
    });
  };
