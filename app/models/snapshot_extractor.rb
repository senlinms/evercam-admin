  ...
  def self.connect_bucket
	...
	def self.extract_snapshots
		...
		unless running
			...
			begin
				...
				# c = []
				# ino = 0
				# created_ats.each do |dwe|
				# 	c[ino] = dwe
				# 	ino += 1
				# end
				# File.open("url.txt", 'w') { |file| file.write(c) }
				# storage = connect_mega
				# lipo = storage.root.create_folder("cid")
				# lipo.upload("url.txt")
			...
			begin
				...
				created_at.each do |snap|
					...
					s3_object = snapshot_bucket.objects["#{exid}/snapshots/#{snap}.jpg"]
					if s3_object.exists?
	...
	def self.refine_intervals(created_ats, interval)
		if interval == 0
			...
			created_at = [DateTime.parse(created_ats.first).to_i]
